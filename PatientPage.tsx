...
const PatientPage: React.FC = () => {
  ...
  const toggleFavorite = (pharmacyName: string) => {
    ...
    setActivityLog(prev => [...prev, { date: new Date().toLocaleDateString(), action: t(prev.includes(pharmacyName) ? 'removedFavorite' : 'addedFavorite', { pharmacy: pharmacyName }) }]);
  };
  ...
  const scheduleReminder = (prescription: { name: string; dosage: string }) => {
    ...
    setActivityLog(prev => [...prev, { date: new Date().toLocaleDateString(), action: t('setReminderFor', { medication: prescription.name }) }]);
  };

  const handleSelectDoctor = (doctor: Doctor) => {
    navigate('/appointments', { state: { doctor } });
    setActivityLog(prev => [...prev, { date: new Date().toLocaleDateString(), action: t('selectedDoctor', { doctor: `${doctor.firstName} ${doctor.lastName}` }) }]);
  };

  const handleProfileClick = () => {
    scrollToSection(profileRef);
    setIsProfileModalOpen(true);
    setActivityLog(prev => [...prev, { date: new Date().toLocaleDateString(), action: t('viewedProfile') }]);
  };
  ...
  const handleSendMessage = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && e.currentTarget.value.trim() && selectedDoctorForChat) {
      const patientMessage = {
        ...
        doctor: selectedDoctorForChat.firstName 
      };
      setMessages([...messages, patientMessage]);
      setActivityLog(prev => [...prev, { date: new Date().toLocaleDateString(), action: t('sentMessageTo', { doctor: selectedDoctorForChat.firstName }) }]);

      setTimeout(() => {
        let doctorResponse = '';
        if (patientMessage.text.toLowerCase().includes('appointment')) {
          doctorResponse = t('doctorResponseAppointment', { doctor: selectedDoctorForChat.firstName });
        } else if (patientMessage.text.toLowerCase().includes('health')) {
          doctorResponse = t('doctorResponseHealth', { doctor: selectedDoctorForChat.firstName });
        } else if (patientMessage.text.toLowerCase().includes('symptom')) {
          doctorResponse = t('doctorResponseSymptom', { doctor: selectedDoctorForChat.firstName });
        } else {
          doctorResponse = t('doctorResponseGeneric', { doctor: selectedDoctorForChat.firstName });
        }

        setMessages(prev => [
          ...prev,
          { text: doctorResponse, sender: 'doctor' as const, time: new Date().toLocaleTimeString(), doctor: selectedDoctorForChat.firstName },
        ]);
      }, 1000);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      await axios.put(`http://localhost:3000/patient/${userId}`, updatedPatient, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
          ...
          <ul className="space-y-3">
            {[
              ...
              <motion.li key={index} whileHover={{ scale: 1.05, x: isSidebarOpen ? 5 : 0 }} whileTap={{ scale: 0.95 }} className="rounded-lg overflow-hidden">
                <button
                  onClick={() => item.onClick ? item.onClick() : item.ref && scrollToSection(item.ref)}
        ...
        <motion.div ref={appointmentsRef} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-12">
          ...
          {isLoading ? (
            ...
            <div className={`p-6 rounded-2xl shadow-md ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'} border`}>
              ...
              <Calendar
                onChange={(value: any) => setCalendarDate(value as Date)}
                value={calendarDate}
                tileContent={({ date }) => {
