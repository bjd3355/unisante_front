...
const SignupPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Remplacez l'URL par l'endpoint de votre API d'inscription
      const response = await axios.post(
        "http://localhost:3000/auth/register",
        { firstName: prenom, lastName: nom,email: email,password: password, role: "patient" },
      ...
      if (axios.isAxiosError(err)) {
        setError(err.response?.data?.message || "Une erreur inconnue s'est produite.");
