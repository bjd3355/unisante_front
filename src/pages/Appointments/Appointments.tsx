...
const AppointmentPage: React.FC = () => {
  ...
  useEffect(() => {
    if (doctor && selectedDate) {
      ...
      .then(response => {
        setBookedAppointments(response.data); // Par exemple : ["08:15", "09:00"]
      })
      .catch(error => {
  ...
  const handleAppointmentConfirm = () => {
    const patientId = localStorage.getItem("userId"); // On suppose que le patient est connecté
              ...
              <>
                ...
                <div className="grid grid-cols-4 gap-2 mb-2">
                  {availableTimes.map((timeSlot) => {
                    const isBooked = bookedAppointments.includes(timeSlot);
                    return (
                      <button 
                        key={timeSlot} 
                        disabled={isBooked}
